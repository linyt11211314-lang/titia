...
export function TitiaApp() {
  ...
  const addFromForm = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (form === "petRecord") { const file=values.petImage instanceof File&&values.petImage.size?values.petImage:null;const image=file?await fileToDataUrl(file):undefined;update((d) => ({ ...d, petRecords: [{ id: uid(), petId: String(values.petId), kind: String(values.kind) as "moment", date: String(values.date), value: String(values.value || ""), note: String(values.note || ""), image }, ...d.petRecords] })); }
  ...
  const page = tab === "today" ? <Today data={data} setData={setData} add={setForm} remove={remove} />
    : tab === "home" ? <HomeSpace data={data} section={homeSection} setSection={setHomeSection} setData={setData} add={setForm} remove={remove} vaultKey={vaultKey} setupOrUnlock={setupOrUnlock} revealVault={revealVault} hideVault={(id:string)=>setVaultVisible((current)=>{const next={...current};delete next[id];return next})} visible={vaultVisible} />
...
function HomeSpace({ data, section, setSection, setData, add, remove, vaultKey, setupOrUnlock, revealVault, hideVault, visible }: any) {
  ...
  return <SectionLayout side={<SideNav items={homeSections} value={section} onChange={setSection} icons={homeIcons}/>}> <Header eyebrow="生活的小小秩序" title={section} action={section !== "密码箱" && <button className="primary small" onClick={() => add(({ 倒数日:"countdown",购物:"shopping",周期:"period",憨憨:data.pets.length?"petRecord":"pet" } as any)[section])}><Plus/>新增</button>}/><div className="content-scroll">
    {section === "倒数日" && <>{data.countdowns.length ? data.countdowns.map((x:any) => {const status=countdownStatus(x.date);return <article className="card countdown" key={x.id}><div className="emoji-tile">{x.category === "生日" ? "🌈" : "✨"}</div><div className="countdown-info"><h3>{x.title||"未命名事件"}</h3><small>{x.category} · {x.calendar === "lunar" ? "农历" : "公历"}{x.repeat ? " · 每年" : ""}</small><p>{status.valid?x.date:"待设置日期"}</p></div><strong>{status.days??"—"}<small>{status.valid?`天 · ${status.label}`:"待完善"}</small></strong><div className="countdown-actions"><button className="icon-button" aria-label={`编辑${x.title}`} onClick={()=>setEditingCountdown(x)}><Settings size={18}/></button><button className="icon-button danger" aria-label={`删除${x.title}`} onClick={()=>remove("countdowns",x.id)}><Trash2 size={18}/></button></div></article>}) : <Empty icon={<Clock3/>} title="期待正在发生" text="记录生日、纪念日和重要时刻"/>}{editingCountdown&&<CountdownEditor item={editingCountdown} close={()=>setEditingCountdown(null)} save={(changed:any)=>{setData((current:AppData)=>({...current,countdowns:current.countdowns.map(item=>item.id===changed.id?changed:item)}));setEditingCountdown(null)}}/>}</>}

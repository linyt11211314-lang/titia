...
import { clampFloatingPosition, snapFloatingPosition } from "./lib/floatingButton";
...
const timeSections = ["日记", "灵光一闪", "关系"] as const;
...
const timeIcons: Record<(typeof timeSections)[number], string> = { 日记: "📖", 灵光一闪: "✨", 关系: "💙" };
  ...
  return <main className="app-shell">
    ...
    <button className={`spark-fab ${sparkEditing ? "editing" : ""}`} aria-label="灵光一闪"
      ...
      onPointerDown={(event) => {
        ...
        sparkTimer.current = window.setTimeout(() => { sparkLongPressed.current = true; setSparkDraft({ ...data.userPreferences.floatingButton }); setSparkEditing(true); }, 1000);
      }}
      onPointerMove={(event) => {
      ...
      onPointerUp={() => { if (sparkTimer.current) window.clearTimeout(sparkTimer.current); if(sparkEditing&&sparkDragging.current&&typeof sparkDraft.x==="number"&&typeof sparkDraft.y==="number"){const snapped=snapFloatingPosition({x:sparkDraft.x,y:sparkDraft.y},{width:window.innerWidth,height:window.innerHeight,buttonSize:58,sideInset:8,topInset:55,bottomInset:126});const saved={...sparkDraft,...snapped};setSparkDraft(saved);setData((current)=>({...current,preferences:{...current.preferences,sparkFab:saved},userPreferences:{...current.userPreferences,floatingButton:saved}}));} sparkDragging.current = false; }}
...
function TimeSpace({data,setData,section,setSection,add,remove}:any){const [relationshipType,setRelationshipType]=useState<"moment"|"review">("moment");const [sparkTag,setSparkTag]=useState("全部");const notes=filterSparkNotes(data.sparkNotes,sparkTag);const actionKind=section==="日记"?"diary":section==="灵光一闪"?"spark":"relationship";return <SectionLayout side={<SideNav items={timeSections} value={section} onChange={setSection} icons={timeIcons}/>}> <Header eyebrow={section==="灵光一闪"?"记录稍纵即逝的想法":"把值得的事留下"} title={section} action={<button className="round-add" aria-label="新增" onClick={()=>add(actionKind)}><Plus/></button>}/><div className="content-scroll">{section==="日记"?<>{data.diaries.map((x:any)=><article className="card diary" key={x.id}><div className="mood">{x.mood}</div><h2>{x.title}</h2><p>{x.body}</p><footer><time>{x.date}</time><DeleteButton onClick={()=>remove("diaries",x.id)}/></footer></article>)}{!data.diaries.length&&<Empty icon={<NotebookTabs/>} title="写给今天" text="留住情绪、照片和一段小故事"/>}</>:section==="灵光一闪"?<><div className="spark-tags">{sparkTags.map(tag=><button key={tag} className={sparkTag===tag?"active":""} onClick={()=>setSparkTag(tag)}>{tag}</button>)}</div>{notes.map((note:any)=><article className="card spark-note" key={note.id}><small>{note.tag}</small><h2>{note.title}</h2><p>{note.content}</p><footer><time>{new Date(note.createdAt).toLocaleString("zh-CN")}</time><button className="text-button" onClick={()=>{const title=prompt("修改标题",note.title);const content=title===null?null:prompt("修改内容",note.content);if(title!==null&&content!==null)setData((current:AppData)=>({...current,sparkNotes:current.sparkNotes.map(item=>item.id===note.id?{...item,title,content,updatedAt:new Date().toISOString()}:item)}))}}>编辑</button><DeleteButton onClick={()=>remove("sparkNotes",note.id)}/></footer></article>)}{!notes.length&&<Empty icon={<Sparkles/>} title="还没有这一类灵光" text="点击右上角或悬浮按钮，快速记下一闪而过的想法"/>}</>:<><div className="relationship-tabs"><button className={relationshipType==="moment"?"active":""} onClick={()=>setRelationshipType("moment")}>💞 感动瞬间</button><button className={relationshipType==="review"?"active":""} onClick={()=>setRelationshipType("review")}>🔎 矛盾复盘</button></div>{data.relationships.filter((x:any)=>x.type===relationshipType).map((x:any)=><article className="card relation" key={x.id}><small>{x.type==="moment"?"💞 感动瞬间":"🔎 矛盾复盘"} · @{x.person}</small><h2>{x.title}</h2><p>{x.body}</p>{x.reflection&&<blockquote>{x.reflection}</blockquote>}<footer><time>{x.date}</time><DeleteButton onClick={()=>remove("relationships",x.id)}/></footer></article>)}{!data.relationships.some((x:any)=>x.type===relationshipType)&&<Empty icon={<Heart/>} title={relationshipType==="moment"?"记录一次被打动":"温柔复盘一次矛盾"} text="两个板块分别保存和查看，不再混在一起"/>}</>}</div></SectionLayout>}
